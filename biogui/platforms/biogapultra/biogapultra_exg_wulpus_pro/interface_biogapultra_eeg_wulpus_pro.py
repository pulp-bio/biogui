...
"""
Combined EEG (ADS1298) + WULPUS PRO interface for BioGAP (nRF5340).

Both sensors stream simultaneously over a single BLE NUS connection.
All BLE packets are exactly 211 bytes; the header byte distinguishes the source:
  0x55        → EEG packet  (4 samples × 2 × 8 channels)
...
# ── Module-level interface contract ───────────────────────────────────────
...
def _build_sig_info(wulpus_cfg: WulpusUssConfig, ads: ads_config.AdsConfig) -> tuple[dict, dict[int, str]]:
    ...
    sig_info["eeg_B"]         = {"fs": eeg_fs, "nCh": _N_CH, "extras": {"type": "time-series"}}
    # ExG packet counter + µs timestamp: selectable in the wizard like eeg_A/eeg_B,
    # but off by default (mirrors the standalone ExG interface).
    sig_info["eeg_counter"]   = {"fs": eeg_packet_rate, "nCh": 1, "extras": {"type": "time-series", "plotByDefault": False}}
    sig_info["eeg_timestamp"] = {"fs": eeg_packet_rate, "nCh": 1, "extras": {"type": "time-series", "plotByDefault": False}}
...
startSeq: list[bytes | float] = [
    ...
    # running alone. EEG starts only once WULPUS's sequence has fully gone out.
    ...
    bytes([START_EEG_STREAMING]) + ads_config.to_bytes(_ads_config),
]
...
def decodeFn(data: bytes) -> dict[str, np.ndarray]:
    """
    Decode one 211-byte packet from the combined EEG+WULPUS stream.

    Header 0x55        → EEG packet: returns eeg_A, eeg_B (4 samples each).
    Header 0x10        → start WULPUS frame accumulation.
    Header 0x11–0x13  → continuation; returns ultrasound data on 0x13.
    """
    global _wulpus_buf
    ...
    if header == _EEG_HEADER:
        ...
        result["eeg_A"]         = rows_A
        result["eeg_B"]         = rows_B
        result["eeg_counter"]   = np.array([[counter]], dtype=np.uint16)
        result["eeg_timestamp"] = np.array([[timestamp]], dtype=np.uint32)
        return result

    # ── WULPUS packets ────────────────────────────────────────────────────
    if header == 0x10:
        if _wulpus_buf:
            logger.warning("EEG+WULPUS: 0x10 before previous frame completed — resyncing")
    ...
    if _wulpus_buf and header == _BLE_WULPUS_HEADERS[len(_wulpus_buf)]:
        ...
        logger.warning("EEG+WULPUS: unexpected header 0x%02X — resyncing", header)
        _wulpus_buf = []
        return result
    ...
    # Full 4-packet WULPUS frame assembled
...
def _configure_eeg_wulpus_module(
    parent: QWidget,
    ...
    current_ads = ads_config.settings_from_sig_info(interface_module.sigInfo, "eeg_A")
    ...
    def _apply() -> None:
        ...
        new_start = [
            ...
            bytes([START_EEG_STREAMING]) + ads_config.to_bytes(new_ads),
        ]
    ...
    if not openDialogShell(
        parent, [ads_widget, wulpus_widget], "EEG + WULPUS PRO Configuration", onAccept=_apply
    ):
        return None
    return configured[0]
...
platformConfig = PlatformConfig(
    id="eeg_wulpus",
    configureInterfaceModule=_configure_eeg_wulpus_module,
...
"""Opens the ADS1298 + WULPUS PRO settings dialog before acquisition, and from
the source's inline configure action."""
