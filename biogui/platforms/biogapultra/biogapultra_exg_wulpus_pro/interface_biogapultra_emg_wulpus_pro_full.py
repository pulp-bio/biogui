...
"""
Combined EMG (ADS1298) + WULPUS PRO interface for BioGAP (nRF5340), Wi-Fi transport.

Identical to interface_biogapultra_emg_wulpus_pro.py except for how the WULPUS
half of the stream is framed: over Wi-Fi one full US frame arrives as a single
815-byte packet (header 0xCC / tailer 0xDD) instead of four 211-byte BLE
chunks, mirroring interface_biogapultra_wulpus_pro_full.py. The EMG half
(header 0x55, 211 bytes) is unchanged -- ADS1298 packets are the same size on
both transports.

Start sequence:
  1. START_WULPUS_STREAMING (41)
  2. WULPUS restart package  (MSP430 reset)
  3. WULPUS conf package     (MSP430 config + start)
  4. START_EMG_STREAMING (37) + 5-byte ADS config, in one message

WULPUS's own sequence goes out first, unmodified and uninterrupted, exactly
as interface_biogapultra_wulpus_pro_full.py sends it -- EMG only starts once
it has fully gone out. Stop mirrors this: WULPUS's stop sequence first, then
EMG's. See interface_biogapultra_emg_wulpus_pro.py's startSeq comment for why
...
# ── Framing constants ──────────────────────────────────────────────────────
_EXG_HEADER = 0x55
_EXG_PACKET_SIZE = 211

# One full US frame arrives as a single packet (see
# interface_biogapultra_wulpus_pro_full.py for the authoritative layout):
#   [0]       header 0xCC
...
_WULPUS_SPI_OFF      = 7    # SPI payload starts here
...
packetSize: list[tuple[int, int]] = [
    (_EXG_HEADER, _EXG_PACKET_SIZE),
    (_WULPUS_FULL_HEADER, _WULPUS_PACKET_SIZE),
]
"""Two packet types on one Wi-Fi link, selected by the first byte."""

wifiPacketSize: list[tuple[int, int]] = packetSize
"""Same sizes over Wi-Fi as natively -- neither packet type is re-wrapped."""
...
startSeq: list[bytes | float] = [
    # WULPUS's own start sequence first, unmodified and uninterrupted -- see
    # module docstring for why. EMG starts only once it has fully gone out.
    bytes([START_WULPUS_STREAMING]),                                # 0x41, 1 byte
    wulpus_config.get_restart_package(),    # MSP430 reset          # 106 bytes
...
def decodeFn(data: bytes) -> dict[str, np.ndarray]:
    """Decode one packet of either type from the combined EMG+WULPUS Wi-Fi stream."""
    header = data[0]
    ...
    if header == _WULPUS_FULL_HEADER:
        tailer = data[-1]
        if tailer != _WULPUS_FULL_TAILER:
            logger.warning(
                f"EMG+WULPUS: unexpected WULPUS tailer 0x{tailer:02X} "
                f"(expected 0x{_WULPUS_FULL_TAILER:02X})"
            )
    ...
    logger.warning("EMG+WULPUS: unexpected header 0x%02X", header)
    return result
...
def _configure_emg_wulpus_module(
    ...
    def _apply() -> None:
        ...
        logger.info(
            "NEW EMG+WULPUS start sequence: %s",
            [item.hex(" ") if isinstance(item, (bytes, bytearray)) else item for item in new_start],
        )
    ...
    if not openDialogShell(
        parent, [ads_widget, wulpus_widget], "EMG + WULPUS PRO Configuration (Wi-Fi)", onAccept=_apply
    ):
        return None
    return configured[0]
