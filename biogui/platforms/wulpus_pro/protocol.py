...
def vga_volts_to_gain_db(input_v):
    return (input_v - 0.1) * (40 / 0.5)


def digipot_code_to_res(code):
    return (1 - code / 256) * DIGIPOT_TOT_RES
...
configuration_package = [
    [
        ...
        _ConfigBytes(
            "enable_env_det", "Envelope detection", "list", (0, 1), ("Disabled", "Enabled"), "<u1"
        ),
...
class WulpusUssConfig:
    ...
    def calc_gain_curve(self):
        # Use actual ultrasound samples, not total samples (which may include IMU)
        num_us_samples = get_num_us_samples_from_config(self)
        self.gain_curve_db = np.zeros(num_us_samples)
        ...
        if inflection_id >= num_us_samples or inflection_id < 0:
            return

        rc_slope = (VGA_RC_SER_RES + digipot_code_to_res(self.vga_slope_code)) * VGA_RC_CAP_VAL
        temp_arr_v = 3.3 * (
            np.arange(num_us_samples - inflection_id) / self.sampling_freq / rc_slope
        )
