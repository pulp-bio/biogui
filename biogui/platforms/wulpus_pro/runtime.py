...
from .wulpus_pro_config_widget import WulpusConfigWidget
...
def create_default_config() -> WulpusUssConfig:
    """Create the default WULPUS PRO configuration used by the bundled interface."""
    ...
    for config_id in range(wulpus_config.num_txrx_configs):
        ...
        sig_info[signal_name] = {
            ...
            "extras": {
                ...
                "wulpus_config": copy.deepcopy(wulpus_config),
            },
        }
...
def configure_interface_module(
    ...
    """Open the WULPUS PRO configuration dialog and return an updated interface module."""
    current_config = read_current_config(interface_module)
    configured_interface_module: InterfaceModule | None = None

    dialog = QDialog(parent)
    dialog.setWindowTitle("WULPUS PRO Configuration")
    ...
    def _apply_and_accept() -> None:
        ...
        try:
            ...
            QMessageBox.critical(
                dialog,
                "Configuration Error",
                f"Invalid WULPUS PRO configuration: {err}",
            )
