...
class _TriggerWidget(QWidget):
    ...
    def renderImage(self, mainText: str, imagePath: str, subText: str = "") -> None:
        ...
        Render:
         ...
         - For plain text (no imagePath): if subText is provided, draw mainText (next label) above and subText (countdown) below.
        ...
        if imagePath:
            ...
            if subText:
                ...
                try:
                    ...
                    painter.setFont(QFont("Arial", 48))
                painter.drawText(pixmap.rect(), Qt.AlignCenter, mainText.upper().replace(" ", "\n"))
...
class TriggerController(QObject):
    """
    Controller for the triggers, now showing the upcoming stimulus label with the countdown during rest (using durationRest in ms).
    """
    ...
    def _updateTriggerAndImage(self) -> None:
        ...
        if self._restFlag:
            ...
            logging.info(f"Rest started: upcoming='{self._upcomingLabel}', countdown={self._restCounter}s (durationRest={restMs}ms).")
    ...
    def _updateCountdown(self) -> None:
        ...
        if self._restCounter > 0:
            self._triggerWidget.renderImage(self._upcomingLabel, "", str(self._restCounter))
            logging.info(f"Rest countdown: upcoming='{self._upcomingLabel}', {self._restCounter}s remaining.")
