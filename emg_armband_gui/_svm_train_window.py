...
class SVMWindow(QWidget, Ui_SVMTrain):
    ...
    def _SVMtrain(self):
        ...
        for label in np.unique(labels):
            if label == 0:
                X_train, X_test, y_train, y_test = train_test_split(WL_signal[labels==label], labels[labels==label], test_size=0.5, random_state=42)
            else:
                X_train1, X_test1, y_train1, y_test1 = train_test_split(WL_signal[labels==label], labels[labels==label], test_size=0.5, random_state=42)
